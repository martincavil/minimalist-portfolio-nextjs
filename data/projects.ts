...
export const projects: Project[] = [
  {
    ...
    work: {
      fr: "J'ai conçu et développé une plateforme d'acquisition complète, entièrement branchée sur Strapi 5 (CMS headless) avec ISR pour les performances. L'architecture SEO programmatique est en cours de déploiement et générera 120 landing pages géolocalisées (15 villes × 8 spécialités), chacune avec metadata unique, JSON-LD contextualisé et maillage interne automatique — prévu pour les prochains mois. Une estimation IA instantanée via l'API Claude Vision (Anthropic) est également en préparation, avec un système de contrôle des coûts pensé dès la conception : email obligatoire avant analyse, rate limiting par IP et global, redimensionnement image côté client, budget cap configurable. Un script de génération d'articles SEO automatisera la création de contenu via l'API Claude. L'infrastructure DNS a été configurée de A à Z : domaine OVH, Vercel, Strapi Cloud Essential, et Resend avec SPF/DKIM/DMARC pour une délivrabilité optimale. Technologies : Next.js 15, TypeScript, Tailwind CSS v4, Strapi 5, Resend, Claude Vision API, GA4 Consent Mode v2, Vercel.",
      en: "I designed and developed a complete lead acquisition platform, fully connected to Strapi 5 (headless CMS) with ISR for performance. The programmatic SEO architecture is being deployed and will generate 120 geolocated landing pages (15 cities × 8 specialties), each with unique metadata, contextualized JSON-LD, and automatic internal linking — planned for the coming months. An instant AI appraisal feature via the Claude Vision API (Anthropic) is also in preparation, with a cost control system designed from the ground up: mandatory email before analysis, per-IP and global rate limiting, client-side image resizing, configurable budget cap. An SEO article generation script will automate content creation via the Claude API. The DNS infrastructure was configured end-to-end: OVH domain, Vercel, Strapi Cloud Essential, and Resend with SPF/DKIM/DMARC for optimal deliverability. Technologies: Next.js 15, TypeScript, Tailwind CSS v4, Strapi 5, Resend, Claude Vision API, GA4 Consent Mode v2, Vercel.",
    },
    results: {
      fr: "Lancement d'une plateforme SEO-first avec une architecture programmatique prête pour 120 pages de longue traîne, couvrant les principales requêtes d'estimation d'objets d'art par ville et spécialité en France — déploiement en cours. Le système d'estimation IA instantanée, en préparation pour les prochains mois, se distinguera dans un marché où aucune maison de ventes concurrente ne propose cette fonctionnalité, chaque analyse capturant l'email du visiteur comme lead qualifié. Le script de génération d'articles permettra une production de contenu SEO à cadence hebdomadaire sans ressource éditoriale supplémentaire.",
      en: "Launch of a SEO-first platform with a programmatic architecture ready for 120 long-tail pages covering the main art object appraisal queries by city and specialty in France — currently being deployed. The instant AI appraisal system, in preparation for the coming months, will stand out in a market where no competing auction house offers this feature, with each analysis capturing the visitor's email as a qualified lead. The article generation script will enable weekly SEO content production without additional editorial resources.",
    },
    KPIs: [
      {
        number: "120 pages",
